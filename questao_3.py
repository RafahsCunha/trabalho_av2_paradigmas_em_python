#3 – Suponha que o programador necessite desenvolver uma calculadora científica que deve possuir, além
#das operações aritméticas básicas (adição, subtração, divisão e multiplicação), a possibilidade de calcular
#raiz quadrada, potenciação de números, funções trigonométricas (seno, cosseno, tangente). Qual biblioteca
#você utilizaria e quais funções você deveria utilizar para a implementação da calculadora? (2,0 pontos). 


# A biblioteca escolhida seria a Tkinter e Math
# Para implementação, utilizaria classes e funções do codigo: 
